...
TEST(AgradRevMatrix, promoter_nan) {
  ...
  double a = promote_common<double,double>(nan);
  var b = promote_common<double,var>(nan);
  var c = promote_common<double,var>(var(nan));
  EXPECT_TRUE(boost::math::isnan(a));
  EXPECT_TRUE(boost::math::isnan(b.val()));
  EXPECT_TRUE(boost::math::isnan(c.val()));

  vector<double> x(5);
  for (int i = 0; i < 5; ++i) x[i] = nan;
  vector<double> y = promote_common<vector<double>, vector<double> >(x);
  EXPECT_EQ(5U,y.size());
  for (int i = 0; i < 5; ++i)
    EXPECT_TRUE(boost::math::isnan(y[i]));
  std::vector<var> z = 
    promote_common<std::vector<double>, std::vector<var> >(x);
  EXPECT_EQ(5U,z.size());
  for (int i = 0; i < 5; ++i)
    EXPECT_TRUE(boost::math::isnan(z[i].val()));
  vector<var> w = promote_common<vector<var>, vector<var> >(z);
  EXPECT_EQ(5U,w.size());
  for (int i = 0; i < 5; ++i)
    EXPECT_TRUE(boost::math::isnan(w[i].val()));
  ...
  EXPECT_EQ(3,V.size());
  for (int i = 0; i < 3; ++i)
    EXPECT_TRUE(boost::math::isnan(V(i)));
  vector_v W = promote_common<vector_d,vector_v>(U);
  EXPECT_EQ(3,W.size());
  for (int i = 0; i < 3; ++i)
    EXPECT_TRUE(boost::math::isnan(W(i).val()));
  ...
  EXPECT_EQ(3,X.size());
  for (int i = 0; i < 3; ++i)
    EXPECT_TRUE(boost::math::isnan(X(i).val()));
  ...
  EXPECT_EQ(3,H.size());
  for (int i = 0; i < 3; ++i)
    EXPECT_TRUE(boost::math::isnan(H(i)));
  row_vector_v I = promote_common<row_vector_d,row_vector_v>(G);
  EXPECT_EQ(3,W.size());
  for (int i = 0; i < 3; ++i)
    EXPECT_TRUE(boost::math::isnan(I(i).val()));
  row_vector_v J = promote_common<row_vector_v,row_vector_v>(I);
  EXPECT_EQ(3,J.size());
  for (int i = 0; i < 3; ++i)
    EXPECT_TRUE(boost::math::isnan(J(i).val()));
  ...
  for (int i = 0; i < 3; ++i)
    for (int j = 0; j < 4; ++j)
      EXPECT_TRUE(boost::math::isnan(B(i,j)));
  matrix_v C = promote_common<matrix_d,matrix_v>(A);
  EXPECT_EQ(3,C.rows());
  EXPECT_EQ(4,C.cols());
  for (int i = 0; i < 3; ++i)
    for (int j = 0; j < 4; ++j)
      EXPECT_TRUE(boost::math::isnan(C(i,j).val()));
  matrix_v D = promote_common<matrix_v,matrix_v>(C);
  EXPECT_EQ(3,D.rows());
  EXPECT_EQ(4,D.cols());
  for (int i = 0; i < 3; ++i)
