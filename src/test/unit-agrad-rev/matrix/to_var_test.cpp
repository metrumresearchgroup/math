...
TEST(AgradRevMatrix,to_var_scalar_nan) {
  double nan = std::numeric_limits<double>::quiet_NaN();
  double d = nan;
  AVAR v = nan;
  stan::agrad::var var_x = stan::agrad::to_var(d);
  EXPECT_TRUE(boost::math::isnan(var_x.val()));

  var_x = stan::agrad::to_var(v);
  EXPECT_TRUE(boost::math::isnan(var_x.val()));
}
TEST(AgradRevMatrix,to_var_matrix_nan) {
  double nan = std::numeric_limits<double>::quiet_NaN();
  using stan::math::matrix_d;
  using stan::agrad::matrix_v;
  matrix_d m_d(2,3);
  ...
  for (int ii = 1; ii < 2; ii++) 
    for (int jj = 0; jj < 3; jj++)
      EXPECT_FLOAT_EQ(ii*3 + jj, m_v(ii, jj).val());
  EXPECT_TRUE(boost::math::isnan(m_v(0,0).val()));
}
TEST(AgradRevMatrix,to_var_vector_nan) {
  using stan::math::vector_d;
  using stan::agrad::vector_v;
  double nan = std::numeric_limits<double>::quiet_NaN();
  ...
  vector_v out = stan::agrad::to_var(d);
  EXPECT_TRUE(boost::math::isnan(out(0).val()));
  EXPECT_FLOAT_EQ(2, out(1).val());
  EXPECT_FLOAT_EQ(3, out(2).val());
  EXPECT_FLOAT_EQ(4, out(3).val());
  EXPECT_FLOAT_EQ(5, out(4).val());
...
TEST(AgradRevMatrix,to_var_rowvector_nan) {
  using stan::math::row_vector_d;
  using stan::agrad::row_vector_v;
  double nan = std::numeric_limits<double>::quiet_NaN();
