...
template <typename F>
class PMXOdeintSystem<F, double, double> {
  int step_counter_;
 public:
  const F& f_;
  const double t0_;
  ...
  void operator()(const std::vector<double>& y, std::vector<double>& dy_dt,
    ...
    stan::math::check_size_match("PMXOdeintSystem", "y", y.size(), "dy_dt",
                     dy_dt.size());
  }
 ...
 */
template <typename F>
struct PMXOdeintSystem<F, double, stan::math::var> {
private:
  int step_counter_;
public:
  const F& f_;
  ...
  void operator()(const std::vector<double>& z, std::vector<double>& dz_dt,
    ...
    try {
      ...
      vector<stan::math::var> y_vars(z.begin(), z.begin() + N_);
      ...
      vector<stan::math::var> dy_dt_vars = f_(t, y_vars, theta_vars, x_, x_int_, msgs_);

      stan::math::check_size_match("PMXOdeintSystem", "dz_dt", dy_dt_vars.size(),
                       "states", N_);
 ...
 */
template <typename F>
struct PMXOdeintSystem<F, stan::math::var, double> {
private:
  int step_counter_;
public:
  const F& f_;
  ...
  void operator()(const std::vector<double>& z, std::vector<double>& dz_dt,
    ...
    try {
      ...
      vector<stan::math::var> dy_dt_vars = f_(t, y_vars, theta_dbl_, x_, x_int_, msgs_);
 ...
 */
template <typename F>
struct PMXOdeintSystem<F, stan::math::var, stan::math::var> {
private:
  int step_counter_;
public:
  const F& f_;
