...
namespace torsten {
  template<typename T_model, typename... T_pred,
           typename std::enable_if_t<!stan::is_var<typename T_model::scalar_type>::value >* = nullptr>
  Eigen::VectorXd model_solve_d(const T_model& pkmodel, const double& dt, T_pred const&... pred_pars) { //NOLINT
  ...
  template<typename T_model, typename... T_pred,
    ...
    Eigen::VectorXd model_solve_d(const T_model& pkmodel, const double& amt, const double& rate, const double& ii, const int& cmt, T_pred const&... pred_pars) { // NOLINT
      return pkmodel.solve(amt, rate, ii, cmt);
  }

  template<typename T_model, typename T_amt, typename T_rate, typename T_ii>
  Eigen::VectorXd solve(const T_model& pkmodel, const T_amt& amt, const T_rate& r, const T_ii& ii, const int& cmt) { // NOLINT
      using std::vector;
      using Eigen::VectorXd;
      using Eigen::Matrix;
      using stan::math::value_of;
      ...
      using T_par = typename T_model::par_type;
      std::vector<T_par>& par(pkmodel.par().size());
