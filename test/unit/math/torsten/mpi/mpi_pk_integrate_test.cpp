...
TEST_F(TorstenOdeTest_neutropenia, mpi_rank_exception_par_var) {
  ...
  EXPECT_THROW(torsten::dsolve::pk_integrate_ode_bdf(f, y0_m, t0, ts_m, theta_m_v , x_r_m, x_i_m),
               std::exception);
  EXPECT_THROW(torsten::dsolve::pk_integrate_ode_adams(f, y0_m, t0, ts_m, theta_m_v , x_r_m, x_i_m),
               std::exception);
#ifdef TORSTEN_MPI
  MPI_Barrier(comm);
#endif
