#ifndef TEST_UNIT_MATH_TORSTEN_UTIL_PKMODELTWOCPT2_HPP
#define TEST_UNIT_MATH_TORSTEN_UTIL_PKMODELTWOCPT2_HPP
...
/**
 * Calculates finite difference for PKModelTwoCpt22 with varying parameters.
...
Eigen::Matrix <double, Eigen::Dynamic, Eigen::Dynamic>
  finite_diff_params(const std::vector<double>& time,
    ...
    pk_res_ub = torsten::PKModelTwoCpt2(time, amt, rate, ii, evid, cmt, addl, ss,
                              pMatrix_ub, biovar_ub, tlag_ub);
    pk_res_lb = torsten::PKModelTwoCpt2(time, amt, rate, ii, evid, cmt, addl, ss,
...
/**
 * Test PKModelTwoCpt2 with only pMatrix as vars and all other continuous
 * arguments as double.
 */
void test_PKModelTwoCpt2_finite_diff_vdd(
  ...
  ode_res = torsten::PKModelTwoCpt2(time, amt, rate, ii, evid, cmt, addl, ss,
    ...
    for (int j = 0; j < nCmt; j++) {
      ...
      for (size_t k = 0; k < parmRows; k++)
        for (size_t l = 0; l < parmCols; l++) {
          ...
          << "Gradient of PKModelTwoCpt2 failed with known"
...
/**
 * Test PKModelTwoCpt2 with only biovar as vars and all other continuous
 * arguments as double.
 */
void test_PKModelTwoCpt2_finite_diff_dvd(
    const std::vector<double>& time,
    const std::vector<double>& amt,
    const std::vector<double>& rate,
    const std::vector<double>& ii,
...
/**
 * Test PKModelTwoCpt2 with only tlag as vars and all other continuous
 * arguments as double.
 ...
 */
void test_PKModelTwoCpt2_finite_diff_ddv(
    const std::vector<double>& time,
    const std::vector<double>& amt,
    const std::vector<double>& rate,
    const std::vector<double>& ii,
...
void test_PKModelTwoCpt2(const std::vector<double>& time,
                        const std::vector<double>& amt,
                        const std::vector<double>& rate,
                        const std::vector<double>& ii,
                        const std::vector<int>& evid,
                        ...
                        const double& diff,
                        const double& diff2) {
  test_PKModelTwoCpt2_finite_diff_vdd(time, amt, rate, ii, evid,
                                    cmt, addl, ss, pMatrix, biovar, tlag,
                                    diff, diff2);
  test_PKModelTwoCpt2_finite_diff_dvd(time, amt, rate, ii, evid,
                                    cmt, addl, ss, pMatrix, biovar, tlag,
                                    diff, diff2);
  test_PKModelTwoCpt2_finite_diff_ddv(time, amt, rate, ii, evid,
                                    cmt, addl, ss, pMatrix, biovar, tlag,
                                    diff, diff2);
}
